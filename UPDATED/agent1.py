...
def main():
	...
	agent=TD3_Agent(0.001,0.001,input_dims=[env.state_dim,],tau=0.005,gamma=0.99,update_actor_interval=2,
		n_actions=env.action_dim,max_size=20000,layer1_size=32,layer2_size=32,batch_size=100,noise=0.2,ID="Level1-1",warmup=12500,chk_dir='tmp/td3/lvl1')
	try:
		last_episode=np.load('Episode.npz')['array1'][0]
		agent.load_models()
		agent.load_state()
		rwdarr=np.load('reward1.npz')['array1']
	...
	for i in range(last_episode,Episodes):
		...
		if i%10==0:
			np.savez('reward1.npz',array1=rwdarr)
